@auth
	<flux:dropdown position="bottom" align="start" {{ $attributes }}>
		<flux:sidebar.profile
			data-test="sidebar-menu-button" :name="auth()->user()->name" :initials="auth()->user()->initials()"
...
@else
	<flux:button class="items-center" :href="route('login')" variant="ghost" wire:navigate {{ $attributes }}>
		{{ __('Đăng nhập') }}
	</flux:button>
@endauth
